...
data = pd.read_csv("181798082.csv")
...
plt.figure()
plt.subplot(2,3,1)
sns.distplot(data['duration']/60.0)
...
most_played50 = hero_mostplayed[:51]
most_played50

hero_winloss_50 = data.groupby('localized_name')['Results'].mean()[most_played50.index].sort_values(ascending=True)
hero_winloss_50



hero_winloss = data.groupby('localized_name')['Results'].mean().sort_values(ascending=True)
hero_winloss
